...
def inject_preview_button(payload, username):
    ...
    injection = f"""<a class="vibe-preview-button" href="/{quoted_user}/preview/" target="_blank" rel="noopener">Open Web App</a><style>.vibe-preview-button{{position:fixed;right:18px;top:14px;z-index:2147483647;border:0;border-radius:999px;background:#d7ff5f;color:#07110d!important;padding:11px 16px;font:800 14px/1.1 sans-serif;text-decoration:none!important;box-shadow:0 10px 30px rgba(0,0,0,.35)}}.vibe-preview-button:hover{{filter:brightness(1.05)}}</style><script>(function(){{function install(){{var term=window.shellinabox;if(!term||!term.input||term.__vibeImePatch){{setTimeout(install,100);return;}}term.__vibeImePatch=true;term.__vibeComposing=false;term.__vibeJustComposed=false;var input=term.input;var originalCheck=term.checkComposedKeys;if(typeof originalCheck==='function'){{term.checkComposedKeys=function(event){{if(term.__vibeComposing||term.__vibeJustComposed){{input.value='';return;}}return originalCheck.call(term,event);}};}}['keyDown','keyPressed','keyUp'].forEach(function(name){{var original=term[name];if(typeof original==='function'){{term[name]=function(event){{if(term.__vibeComposing||term.__vibeJustComposed||(event&&(event.isComposing||event.keyCode===229))){{return false;}}return original.call(term,event);}};}}}});input.addEventListener('compositionstart',function(){{term.__vibeComposing=true;term.__vibeJustComposed=false;}},false);input.addEventListener('compositionend',function(event){{var text=event.data||input.value||'';term.__vibeComposing=false;term.__vibeJustComposed=true;input.value='';setTimeout(function(){{input.value='';term.__vibeJustComposed=false;}},160);if(text&&typeof term.keysPressed==='function'){{term.keysPressed(text);}}}},false);}}install();}})();</script>"""
