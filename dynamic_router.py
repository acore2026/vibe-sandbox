...
def inject_preview_button(payload, username):
    ...
    injection = f"""<a class="vibe-preview-button" href="/{quoted_user}/preview/" target="_blank" rel="noopener">Open Web App</a><style>.vibe-preview-button{{position:fixed;right:18px;top:14px;z-index:2147483647;border:0;border-radius:999px;background:#d7ff5f;color:#07110d!important;padding:11px 16px;font:800 14px/1.1 sans-serif;text-decoration:none!important;box-shadow:0 10px 30px rgba(0,0,0,.35)}}.vibe-preview-button:hover{{filter:brightness(1.05)}}</style><script>(function(){{function install(){{var term=window.shellinabox;if(!term||!term.input||term.__vibeImePatch){{setTimeout(install,100);return;}}term.__vibeImePatch=true;term.__vibeComposing=false;var input=term.input;var originalCheck=term.checkComposedKeys;term.checkComposedKeys=function(event){{if(term.__vibeComposing){{input.value='';return;}}return originalCheck.call(term,event);}};function stop(event){{event.stopImmediatePropagation();event.stopPropagation();if(event.cancelable!==false){{event.preventDefault();}}input.value='';return false;}}input.addEventListener('compositionstart',function(event){{term.__vibeComposing=true;input.value='';event.stopImmediatePropagation();}},true);input.addEventListener('compositionupdate',function(event){{input.value='';event.stopImmediatePropagation();}},true);input.addEventListener('compositionend',function(event){{var text=event.data||input.value||'';term.__vibeComposing=false;input.value='';event.stopImmediatePropagation();if(event.cancelable!==false){{event.preventDefault();}}if(text){{term.keysPressed(text);}}}},true);['keydown','keypress','keyup','beforeinput','input'].forEach(function(type){{input.addEventListener(type,function(event){{if(term.__vibeComposing||event.isComposing||event.keyCode===229||(event.inputType&&event.inputType.indexOf('Composition')!==-1)){{return stop(event);}}}},true);}});}}install();}})();</script>"""
    return html_payload.replace(marker, injection + marker, 1).encode("utf-8")
